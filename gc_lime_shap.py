...
"""# 2. Implementação do Grad-CAM++ COM SEGMENTAÇÃO NO CÓDIGO
...
"""# 2. Implementação do Grad-CAM++ PRÉ SEGMENTADA
...
"""# 3. Implementação do LIME COM SEGMENTAÇÃO NO CÓDIGO
...
"""# 3. Implementação do LIME PRÉ SEGMENTADA
...
# Cria pasta para salvar
output_folder = f'/content/drive/My Drive/Projeto/LIME_Resultados_Para_Artigo/{CLASSE_ALVO}/'
if not os.path.exists(output_folder):
    os.makedirs(output_folder)
...
def processar_classe_lime(nome_classe):
    ...
    lista_atual = banco_imagens[nome_classe]
    output_folder = f'/content/drive/My Drive/Projeto/LIME_Resultados_Para_Artigo/{nome_classe}/'
...
"""# 4. Implementação do SHAP COM SEGMENTAÇÃO NO CÓDIGO
...
# --- 5. CONFIGURAÇÃO DA EXPORTAÇÃO COM SEGMENTAÇÃO ---
# ==========================================
# ESCOLHA A CLASSE AQUI
# #Normal #Glioma #Meningioma #Pituitary
# ==========================================
...
output_dir = f'/content/drive/My Drive/Projeto/SHAP_Resultados/{CLASSE_ALVO}/'
...
# --- 7. LOOP PARA PROCESSAR E SALVAR AS IMAGENS DA CLASSE ALVO ---
for i, caminho_teste in enumerate(banco_imagens[CLASSE_ALVO]):
    ...
    # max_evals=500 é um limite seguro. Para resultados ainda mais detalhados, pode aumentar para 1000 ou 2000. Só leva mais tempo.
    shap_values = explainer(img_teste_batch, max_evals=500, batch_size=50)

    # C. Gerar e Salvar a Imagem
    # show=False é crucial para evitar que a imagem pare o loop
    shap.image_plot(shap_values, show=False)

    # Captura a figura atual gerada pelo matplotlib
    fig = plt.gcf()

    # Ajusta o tamanho da figura (18x5 mantém aquela proporção esticada legal do SHAP)
    fig.set_size_inches(18, 5)

    # Salva a imagem com alta qualidade (300 dpi) e cortando bordas em branco extras
    caminho_salvar = os.path.join(output_dir, f"SHAP_{nome_arquivo}.png")
    plt.savefig(caminho_salvar, bbox_inches='tight', dpi=300)

    # Limpa a memória fechando a figura (muito importante dentro de loops)
    plt.close()
...
"""# 4. Implementação do SHAP PRÉ SEGMENTADA"""
...
# --- 1. FUNÇÃO DE SEGMENTAÇÃO OTIMIZADA ---
def segment_image_cv(img_array):
    # Converte para uint8 para o OpenCV
    img = img_array.astype(np.uint8)
    if len(img.shape) == 4: img = np.squeeze(img, axis=0)
...
classes = list(banco_imagens.keys())
n_classes = len(classes)
n_imagens = 5 # quantidade de imagens por classe no seu dicionário

fig, axes = plt.subplots(n_classes, n_imagens, figsize=(20, 15))
plt.subplots_adjust(wspace=0.1, hspace=0.3)

# --- 3. LOOP DE PROCESSAMENTO ---
for row, classe in enumerate(classes):
    for col, caminho in enumerate(banco_imagens[classe]):
        # Carregar imagem
        img = tf.keras.preprocessing.image.load_img(caminho, target_size=(128, 128))
        img_array = tf.keras.preprocessing.image.img_to_array(img)

        # Aplicar segmentação
        img_seg = segment_image_cv(img_array)

        # Exibir no grid
        ax = axes[row, col]
        ax.imshow(img_seg.astype(np.uint8))
        if col == 0:
            ax.set_ylabel(classe, fontsize=16, fontweight='bold')
        ax.set_xticks([])
        ax.set_yticks([])
        ax.set_title(f"Amostra {col+1}")
